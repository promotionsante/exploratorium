...
```{r function-create_skeleton_dic}
#' Create the skeleton of a dictionnary dataset
#' 
...
```{r function-get_back_arrow_html}
...
```{r example-get_back_arrow_html}
get_back_arrow_html()
```
  
```{r tests-get_back_arrow_html}
...
```{r function-log_all_current_module_input}
#' Log values for all input inside the current module
#' 
...
```{r example-log_all_current_module_input}
...
```{r tests-log_all_current_module_input}
